...
} from "@/services/historyCategoryService";
...
const { categories, fetchCategories: loadCategories } =
  useOptionHistoryCategories();
...
const loadUserData = async () => {
  ...
  if (sessionUser) {
    user.value = {
      ...
      is_public: sessionUser.is_public || false,
    };
  }
};
...
const openEditProfile = () => {
  ...
  editIsPublic.value = user.value.is_public;
...
const saveProfile = async () => {
  // Set a timeout to reload the page after 3 seconds if the operation is still ongoing
  ...
  try {
    ...
    let finalAvatarUrl =
      editAvatar.value === "/img.jpg" ? "" : editAvatar.value;

    if (avatarFile.value && finalAvatarUrl !== "/img.jpg") {
    ...
    setTimeout(() => {
      showEditProfile.value = false;
      successMessage.value = "";
      clearTimeout(reloadTimer); // Clear the reload timer since we completed successfully
    }, 1500);
  } catch (error: any) {
    errorMessage.value = error.message || "Failed to update profile";
    toast.error(errorMessage.value);
    clearTimeout(reloadTimer); // Clear the reload timer on error
  } finally {
    showEditProfile.value = false;
    successMessage.value = "";
    toast.dismiss();
...
const handleBannerSubmit = async (data: { name: string; url: string }) => {
  try {
    if (selectedBanner.value) {
      const { error } = await bannerService.update(
...
const handleCategorySubmit = async () => {
  ...
  try {
    if (selectedCategory.value) {
      const { error } = await historyCategoryService.update(
      ...
      const { error } = await historyCategoryService.create({
        name: categoryName.value,
      });
      if (error) throw error;
...
<template>
  <div class="">
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-2xl w-full max-w-md p-6 space-y-4">
          ...
          <div
            v-if="successMessage"
            class="bg-green-50 text-green-600 p-3 rounded-lg text-sm"
          >
            {{ successMessage }}
          </div>
          ...
          <div class="space-y-4">
            <div class="flex flex-col items-center">
              <div class="relative">
                ...
                />
                <label
                  class="absolute bottom-0 right-0 bg-white p-1.5 rounded-full shadow-md border border-gray-200 cursor-pointer hover:bg-gray-50"
                >
                  <svg
            ...
            <div class="relative inline-flex items-center mt-2">
              ...
              <label
                for="publicActive"
                class="toggle-bg rounded-full p-1.5 w-16 transition-colors duration-200"
                :class="{
                  'bg-green-500': editIsPublic,
                  'bg-red-500': !editIsPublic,
                }"
              >
                <div
                  class="toggle-indicator bg-white rounded-full w-6 h-6 shadow-sm transform transition-transform ease-in-out duration-200"
                  :class="{
                    'translate-x-8': editIsPublic,
                    'translate-x-0': !editIsPublic,
                  }"
                ></div>
              </label>
              <label
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white rounded-2xl w-full max-w-md p-6 max-h-[80vh] flex flex-col"
        >
